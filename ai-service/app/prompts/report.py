...
- Транзакции с категорией UNKNOWN помещай отдельно, НЕ в taxable_events и НЕ в non_taxable_events
- Для них добавь отдельное поле "requires_review" со списком хэшей и пояснением что пользователь должен проверить их вручную
- Транзакции с confidence < 0.75 или категорией UNKNOWN НЕ включай в налогооблагаемую базу автоматически
- Для таких транзакций добавь поле "requires_review_warning" с текстом что пользователь должен проверить их вручную
- Пользователь активно торгует или просто хранит?
- Есть ли признаки работы с DeFi?
- Насколько рискованный профиль с точки зрения налоговой?
...
Формат ответа:
{
  ...
  "requires_review_warning": "текст предупреждения если есть спорные транзакции",
  "requires_review_hashes": ["0xabc...", "0xdef..."]
}"""
